...
function Cards() {
  ...
  const openCase = (index: number) => {
    ...
    cardPlaceHolder.current[index]!.style.height =
      `${firstProjectCard.current!.height}px`;
    cardPlaceHolder.current[index]!.style.width =
      `${firstProjectCard.current!.width}px`;
    setOpenIndex(index);
  };
  ...
  useLayoutEffect(() => {
    if (!isClosing) {
      if (
        !firstProjectCard.current ||
        openIndex === null ||
        !projectCard.current[openIndex]
      )
        return;
      const lastProjectCard =
        projectCard.current[openIndex].getBoundingClientRect();
      ...
      projectCard.current[openIndex].style.transform =
    ...
    if (isClosing) {
      ...
      projectCard.current[openIndex].style.height =
        `${firstProjectCard.current.height}px`;
      projectCard.current[openIndex].style.top =
        `${firstProjectCard.current.top + window.scrollY}px`;
      projectCard.current[openIndex].style.left =
        `${firstProjectCard.current.left}px`;
      ...
      requestAnimationFrame(() => {
        ...
        projectCard.current[openIndex]!.style.top =
          `${placeholderPosition.top + window.scrollY}px`;
        projectCard.current[openIndex]!.style.left =
          `${placeholderPosition.left}px`;
        projectCard.current[openIndex]!.style.transition =
          "width 0.4s ease, height 0.4s ease, left 0.4s ease, top 0.4s ease";
        projectCard.current[openIndex]!.style.width =
          `${placeholderPosition.width}px`;
        setIsClosing(false);
        setOpenIndex(null);
      });
    }
          ...
          >
            <section className={styles.hook}>
              <div className={styles.header}>
                <div className={styles.titles}>
                  <p className={`${styles.type} tech secondary-text`}>
                    {card.type}
                  </p>
              ...
              <p
                ...
                className={`${styles.description} body`}
              >
                {card.hook}
              </p>
            ...
            <section className={`${styles.buttons} `}>
              <button
                ...
                className={`${styles.button} ${styles.caseStudyBtn} tech secondary-text`}
              >
                Case Study
              </button>
              <a
                ...
                className={`${styles.button} ${styles.demo} tech`}
              >
                Demo
              </a>
            </section>
